...
5. **type 类型选择规则**（按优先级从高到低，只能选一个最匹配的）：
   ...
   - docs: 文档变更（涉及 README、USAGE、*.md 等文档类文件的修改添加，描述开头用 docs）
...
请以 JSON 格式返回：
{
  ...
  "params": {
    "message": "Commit message (MUST be in English, conventional commit style. Type: fix>feat>refactor>perf>docs>style>test>chore. fix=bug修复, feat=源码变更(*.go/*.py/*.ts等), docs=文档(README/USAGE/*.md). Be specific, NOT vague like 'update files')",
    "file": "文件名（如适用）",
    "author": "作者名（如适用）",
    "branch": "分支名（如适用）",
    "version_id": "版本ID（如适用）",
...
- git_commit: 创建提交（参数: message=commit message in English, conventional commit style. Type: fix>feat>refactor>perf>docs>style>test>chore）
