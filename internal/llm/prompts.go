...
3. **总结具体修改内容**，避免笼统描述。例如：
   - ❌ 差: "docs: update documentation files and code"（太笼统，不知道更新了什么）
   - ❌ 差: "chore: save changes"（完全没说改了什么）
   - ✅ 好: "docs: update CLI prompt format and add commit_hash param to view_diff"
   - ✅ 好: "feat: add HTTPS authentication support for push operations"
   - ✅ 好: "fix: resolve merge conflict detection in Diff method"
4. 使用 conventional commit 风格，格式为 type: summary
   - type 包括：feat（新功能）、fix（修复）、docs（文档）、refactor（重构）、chore（杂项）
5. 主要修改写在 summary 中，少量关键细节可用括号补充
6. 如果修改涉及多个方面，按重要性排列，不必逐个文件罗列`
...
请以 JSON 格式返回：
{
  ...
  "params": {
    "message": "Commit message (MUST be in English, conventional commit style. MUST be based on actual diff content — be specific about what changed, NOT vague like 'update files')",
    "file": "文件名（如适用）",
    "author": "作者名（如适用）",
    "branch": "分支名（如适用）",
    "version_id": "版本ID（如适用）",
